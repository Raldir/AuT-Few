...
from .adaptation_layers import IA3Linear, IA3LinearEnsemble, IA3LoRALinear, LoRALinear, LoRALinearEnsemble, IA3LoRAEnsembleLinear
...
def inject_ia3_ensemble_to_linear_layer(
    model: nn.Module,
        ...
        if not module_filter or any(re.match(filter_module, m_name) for filter_module in module_filter):
            for c_name, layer in dict(module.named_children()).items():
                if not filter or any(re.match(filter_layer, c_name) for filter_layer in filter):
                    ...
                    lora_layer = IA3LinearEnsemble(layer.in_features, layer.out_features, num_ensemble)
                    lora_layer.weight = layer.weight
                    lora_layer.bias = layer.bias
                    setattr(module, c_name, lora_layer)
                    lora_layers.append(lora_layer)

    setattr(model, "layers_to_update", lora_layers)
    return [model, lora_layers]  # return model to enable method chaining



def inject_ia3_lora_ensemble_to_linear_layer(
    model: nn.Module,
    lora_r: int,
    lora_alpha: int,
    filter: Optional[List[str]] = None,
        ...
        if not module_filter or any(re.match(filter_module, m_name) for filter_module in module_filter):
            for c_name, layer in dict(module.named_children()).items():
                if not filter or any(re.match(filter_layer, c_name) for filter_layer in filter):
                    ...
                    lora_layer = IA3LoRAEnsembleLinear(
                        layer.in_features, layer.out_features, r=lora_r, lora_alpha=lora_alpha, merge_weights=False, num_ensemble=num_ensemble)
                    lora_layer.weight = layer.weight
                    lora_layer.bias = layer.bias
                    setattr(module, c_name, lora_layer)
                    lora_layers.append(lora_layer)
...
def inject_lora_ensemble_to_linear_layer(
    model: nn.Module,
    lora_r: int,
    lora_alpha: int,
    filter: Optional[List[str]] = None,
...
def inject_ia3_lora_to_linear_layer(
    model: nn.Module,
    lora_r : int,
    lora_alpha : int,
    filter: Optional[List[str]] = None,
        ...
        if not module_filter or any(re.match(filter_module, m_name) for filter_module in module_filter):
            for c_name, layer in dict(module.named_children()).items():
                if not filter or any(re.match(filter_layer, c_name) for filter_layer in filter):
                    ...
                    lora_layer = IA3LoRALinear(layer.in_features, layer.out_features)
                    lora_layer.weight = layer.weight
                    lora_layer.bias = layer.bias
                    setattr(module, c_name, lora_layer)
                    lora_layers.append(lora_layer)
