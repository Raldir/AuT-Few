...
from .models.utils import inject_ia3_lora_to_linear_layer, inject_ia3_to_linear_layer, inject_ia3_ensemble_to_linear_layer, inject_lora_to_linear_layer, inject_lora_ensemble_to_linear_layer, inject_ia3_lora_ensemble_to_linear_layer
...
def create_model(
    ...
    for model_name in names:
        ...
        if OmegaConf.select(config, "optimization.efficient_finetune"):
            model = apply_model_adaptation(model, config, current_iteration, save_path, model_config.checkpoint_name)
...
def apply_model_adaptation(model: nn.Module, config: DictConfig, current_iteration, save_path, checkpoint_name) -> nn.Module:
    """
...
def load_pretrained_weights(model, save_path, checkpoint_name, current_iteration, ensemble=False, load_pretrained=False, pretrained_name=None):
    # TODO: Download model automatically if not available
    if current_iteration > 0:
        # trainable_states = torch.load(os.path.join(save_path[:-1] + str(current_iteration - 1), "model.ckpt"), map_location=torch.device("cpu"))
        trainable_states = torch.load(os.path.join(save_path + "_copy", "model.ckpt"), map_location=torch.device("cpu"))
        load_result = model.load_state_dict(trainable_states['state_dict'], strict=False)
        assert (
                len(load_result.unexpected_keys) == 0
            ), f"Load model failed, unexpected keys {load_result.unexpected_keys.__str__()}"    
    elif load_pretrained:
        pretrained_names = pretrained_name.split(',') #Consider each checkpoint to be seperated by a comma
        paths = []
        for pretrained_name in pretrained_names:
            if "3B" in checkpoint_name or "tfew" in checkpoint_name or "flan" in checkpoint_name or "BART0" in checkpoint_name:
                path = pkg_resources.resource_filename(__name__, os.path.join('data', pretrained_name))      
            else:   
                path = pkg_resources.resource_filename(__name__, os.path.join('data', 't011b_ia3_finish.pt'))

            if not os.path.exists(path):
                logger.info(f"Downloading pre-trained weights {pretrained_name} for IA3...")
                if "3B" in checkpoint_name or "BART0" in checkpoint_name:
                    if pretrained_name == "t03b_ia3_finish.pt":
                        download(url="https://automl-mm-bench.s3.amazonaws.com/few_shot/pretrained_weights_ia3/t03b_ia3_finish.pt", path=path) #TODO: Add hash
                    else:
                        download(url=os.path.join("https://automl-mm-bench.s3.amazonaws.com/few_shot/pretrained_weights_ia3/custom_weights/pretrained_weights_" + pretrained_name.split("_global_step")[0], "global_step" + pretrained_name.split("global_step")[1]), path=path) #TODO: Add hash
                else:
                    download(url="https://automl-mm-bench.s3.amazonaws.com/few_shot/pretrained_weights_ia3/t011b_ia3_finish.pt", path=path) #TODO: Add hash
            paths.append(path)
        replace_weights(model, paths, ensemble=ensemble)


def replace_weights(model, weight_path, ensemble):
    assert len(weight_path) > 0, "Weight path is empty"

    if len(weight_path) == 1:
        weight_path = weight_path[0]
        trainable_states = torch.load(weight_path, map_location=torch.device("cpu"))
        new_trainable_states = {}

        for key, value in trainable_states.items():
            if ensemble:
                if "lora" in weight_path:
                    new_trainable_states["model." + key.replace("lora_B", "lora_B_old_0").replace("lora_A", "lora_A_old_0")] = copy.deepcopy(value)
                else:
                    new_trainable_states["model." + key.replace("multi_lora_b", "lora_b_old_0")] = copy.deepcopy(value)
            else:
                if "lora" in weight_path:
                    new_trainable_states["model." + key.replace("lora_B", "lora_B").replace("lora_A", "lora_A")] = copy.deepcopy(value)
                else:
                    new_trainable_states["model." + key.replace("multi_lora_b", "lora_b")] = copy.deepcopy(value)
        
        trainable_states = new_trainable_states

        load_result = model.load_state_dict(trainable_states, strict=False)
        assert (
            len(load_result.unexpected_keys) == 0
        ), f"Load model failed, unexpected keys {load_result.unexpected_keys.__str__()}"
    else:
        for i, weight in enumerate(weight_path):
            trainable_states = torch.load(weight, map_location=torch.device("cpu"))
            new_trainable_states = {}

            for key, value in trainable_states.items():
                if ensemble:
                    if "lora" in weight:
                        new_trainable_states["model." + key.replace("lora_B", "lora_B_old_"  + str(i)).replace("lora_A", "lora_A_old_" + str(i))] = copy.deepcopy(value)
                    else:
                        new_trainable_states["model." + key.replace("multi_lora_b", "lora_b_old_" + str(i-1))] = copy.deepcopy(value) #FIIIx
                else:
                    if "lora" in weight:
                        new_trainable_states["model." + key.replace("lora_B", "lora_B").replace("lora_A", "lora_A")] = copy.deepcopy(value)
                    else:
...
def compute_score(
    ...
    if metric.name in [ROC_AUC, AVERAGE_PRECISION]:
        ...
        return f1_score(metric_data[Y_TRUE], metric_data[Y_PRED], pos_label=pos_label, average=averaging) 
    else:
