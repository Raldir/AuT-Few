...
class MultiModalPredictor:
    ...
    def fit(
        ...
        train_data_or = copy.deepcopy(train_data)
    ...
    def generate_and_add_choice(
        ...
        **hpo_kwargs,
        ...
        task = LitModule(
            ...
            save_path=save_path,
            top_k = OmegaConf.select(config, "optimization.top_k"),
            **metrics_kwargs,
            **optimization_kwargs,
        )
        ...
        if config.env.strategy == 'deepspeed_stage_3':
        ...
        with apply_log_filter(log_filter):
            trainer = pl.Trainer(
                gpus=num_gpus,  # ray lightning requires not specifying gpus
        ...
        with warnings.catch_warnings():
            ...
            model.compute_choice_statistics()

            template_engine.add_possible_choice(" ||| ".join(model.choices))

            # template_engine.best_choices = [" ||| ".join(model.choices)]
            # template_engine.apply_and_save_best_choices(save_path)
            model.calibrate_choices = False

            model.cpu()
            del model, trainer, task, train_dm_pred
            gc.collect()
    ...
    def _fit(
        ...
        **hpo_kwargs,
        ...
        with warnings.catch_warnings():
            ...
            if template_engine.tune_templates:

                template_engine = self.generate_and_add_choice(
                train_df=train_df,
                val_df=val_df,
                validation_metric_name=validation_metric_name,
                minmax_mode=minmax_mode,
                ...
                if restrict_choices != "None":
                    if restrict_choices == "template-only":
                        template_engine.apply_and_save_best_choices(save_path, manual_choice=0)
                    elif restrict_choices == "topic-only":
                        template_engine.apply_and_save_best_choices(save_path, manual_choice=1)
                    else:
                    ...
                    best_scores = []
                    summed_probs = []
                    list_train = train_df.T.to_dict().values()
                    num_classes = len(set([x['label'] for x in list_train]))
                    len_dataframe = train_df.shape[0]

                    sampled_data_1 = []

                    for cl in range(num_classes):
                        subset = train_df.query(f"label == {cl}")
                        len_subset = subset.shape[0]
                        sampled_data_1.append(subset.sample(int(len_subset / 3), random_state=0, replace=False))
                    sampled_data_1 = pd.concat(sampled_data_1)
                    
                    sampled_data_val = train_df[~train_df.idx.isin(sampled_data_1.idx)]
                    sampled_data_2 = []

                    for cl in range(num_classes):
                        subset = sampled_data_val.query(f"label == {cl}")
                        len_subset = subset.shape[0]
                        sampled_data_2.append(subset.sample(int(len_subset / 2), random_state=0, replace=False))
                
                    sampled_data_2 = pd.concat(sampled_data_2)
                    
                    sampled_data_3 = train_df[~train_df.idx.isin(sampled_data_1.idx) & ~train_df.idx.isin(sampled_data_2.idx)]


                    while(template_engine.apply_next_choice()):
                        score, summed_prob = self.fit_choice_selection(
                            train_df=pd.concat([sampled_data_1, sampled_data_2]),
                            val_df=sampled_data_3,
                            test_df=val_df,
                            validation_metric_name=validation_metric_name,
                            minmax_mode=minmax_mode,
                            max_time=max_time,
                        ...
                        score_2, summed_prob_2 = self.fit_choice_selection(
                            train_df=pd.concat([sampled_data_1, sampled_data_3]),
                            val_df=sampled_data_2,
                            test_df=val_df,
                            validation_metric_name=validation_metric_name,
                            minmax_mode=minmax_mode,
                            max_time=max_time,
                        ...
                        score_3, summed_prob_3 = self.fit_choice_selection(
                            train_df=pd.concat([sampled_data_2, sampled_data_3]),
                            val_df=sampled_data_1,
                            test_df=val_df,
                            validation_metric_name=validation_metric_name,
                            minmax_mode=minmax_mode,
                            max_time=max_time,
                    ...
                    # Sum scores turned into probabilities
                    ...
                    template_engine.best_choices.append(best_choice_space)
                    template_engine.apply_and_save_best_choices(save_path)
        ...
        if strategy == 'deepspeed_stage_3':
            from pytorch_lightning.strategies import DeepSpeedStrategy
            strategy = DeepSpeedStrategy(stage=3, offload_optimizer=True, offload_parameters=False, allgather_bucket_size=1e9, reduce_bucket_size=1e9)
    ...
    def fit_choice_selection(
        ...
        test_dm = BaseDataModule(
            ...
            per_gpu_batch_size=config.env.per_gpu_batch_size,
            num_workers=config.env.num_workers,
            predict_data=test_df.iloc[:1000],
        )
        ...
        with warnings.catch_warnings():
            ...
            def evaluate_template(outputs):
                ...
                y_true = self._df_preprocessor.transform_label_for_metric(df=val_df)
                metric_data.update(
                    {
                        Y_PRED: y_pred,
                        Y_TRUE: y_true,
                ...
                if metrics is None:
                    metrics = [self._eval_metric_name, "f1"]
            ...
            outputs = trainer.predict(task, datamodule=val_dm)

            score = evaluate_template(outputs)   

            outputs_test = trainer.predict(task, datamodule=test_dm)

            logits_or_prob_test = extract_from_output(ret_type=LOGITS, outputs=outputs_test)
            y_pred_prob_test = logits_or_prob_test

            y_pred_prob_test = torch.tensor(y_pred_prob_test)

            summed_probs = -1 * torch.sum(y_pred_prob_test)

            print("SUMMED", summed_probs)

            model.cpu()
            del model, trainer, task, val_dm
            gc.collect()
            torch.cuda.empty_cache()

            return score, summed_probs
    ...
    def _predict(
        ...
        # self._config.env.per_gpu_batch_size = 1
        ...
        # if self._config.env.strategy == 'deepspeed_stage_3':
        #     strategy = DeepSpeedStrategy(stage=3, offload_optimizer=True, offload_parameters=False, allgather_bucket_size=1e9, reduce_bucket_size=1e9)
        if num_gpus > 1:
            strategy = "dp"
