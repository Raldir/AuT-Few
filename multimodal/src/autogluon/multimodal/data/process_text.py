...
from ..constants import AUTOMM, CHOICES_IDS, COLUMN, TEXT, TEXT_SEGMENT_IDS, TEXT_TOKEN_IDS, TEXT_VALID_LENGTH, CURRENT_TEMPLATE
...
class TextProcessor:
    ...
    @property
    def text_token_ids_key_zero_(self):
        return f"{self.prefix}_{TEXT_TOKEN_IDS}_0"
    ...
    @property
    def choices_ids_key_zero(self):
        return f"{self.prefix}_{CHOICES_IDS}_0"
        ...
        fn.update(
            {
                ...
                self.current_template_key: Stack(),
            }
        )

        for i in range(len(self.template_engine.templates)):
            fn.update({
                self.text_token_ids_key + "_" + str(i): Pad(pad_val=self.tokenizer.pad_token_id),
                self.choices_ids_key + "_" + str(i): Pad(pad_val=0),
            })
    ...
    def build_one_token_sequence(
        ...
        for (col_name, txt_token), trim_length in zip(text_tokens.items(), trimmed_lengths):
            # print(CHOICES_IDS, col_name)
            # print(TEXT_TOKEN_IDS, col_name)
            # print("_________________")
            if CHOICES_IDS in col_name:
                choices_ids = txt_token
                ret.update({f"{self.prefix}_" + col_name:choices_ids})
                continue
            elif TEXT_TOKEN_IDS in col_name:
                token_ids.extend(txt_token[0 : (0 + max_length)].tolist())
                if self.insert_sep:
                ...
                ret.update({f"{self.prefix}_" +col_name:copy.deepcopy(token_ids)})
                token_ids = []
                continue
        ...
        ret.update(
            {
                self.text_token_ids_key: np.array([], dtype=np.int32),
        ...
        ret.update(
            {
                self.current_template_key: current_template,
            }
        )

        return ret

    def get_max_choice_length(self, templates):
        text = {}
        max_length = 0
    ...
    def build_one_token_sequence_from_text(
        ...
        # print(self.template_config.num_templates)
        if self.template_config.turn_on:
            ...
            template, applied_template = self.template_engine.sample_and_apply_template(text)
            current_template = self.template_engine.templates.index(template)


            templates, applied_templates = self.template_engine.sample_and_apply_template_all(text)
            if templates:
                max_token_length_choices = self.get_max_choice_length(templates)
                text = {}
                for i, template in enumerate(templates):
                    answer_choices = template.get_answer_choices_list(text)
                    applied_template = applied_templates[i]
                    text[TEXT_TOKEN_IDS + "_" + str(i)] = applied_template[0]
                    text[CHOICES_IDS + "_" + str(i)] = answer_choices
        ...
        for col_name, col_text in text.items():
            ...
            if CHOICES_IDS in col_name:
                answer_ids = self.tokenizer(
                    ...
                    return_tensors="pt",
                    padding="max_length",
                    max_length=max_token_length_choices,
                )["input_ids"]
        ...
        return self.build_one_token_sequence(tokens, current_template)
    ...
    def get_pretrained_tokenizer(
        ...
        if num_templates != None:
            self.template_num = num_templates
