...
class TemplateEngine:
    ...
    def apply_and_save_best_choices(self, save_path, manual_choice=None):
        ...
        choice_index = -1
        choice_index_m = -1
        print("POSSIBLE", self.possible_choices)
        for i, template in enumerate(self.templates):
            ...
            # self.templates[i].answer_choices = self.best_choices[0]
            # self.templates[i].jinja = self.templates_copy[i].jinja.replace("|||", self.possible_choices[self.best_choices[i]].replace("|||", "or") + "?\n|||")
    ...
    def apply_choice_to_template(self, choice):
        ...
        # self.templates[self.restrict_templates].answer_choices = choice

        # self.templates[self.restrict_templates].jinja = self.templates_copy[self.restrict_templates].jinja.replace("|||", choice.replace("|||", "or") + "?\n|||")
        # print(self.templates[self.restrict_templates].jinja)
        # print(self.templates[self.restrict_templates].answer_choices)


    def apply_next_choice(self):
        # print(self.current_choice, len(self.possible_choices))
    ...
    def generate_possible_choices(self, use_dataset_choices=False):
        ...
        if use_dataset_choices:
            ...
            # elif self.preset_templates[1] in ["neurips_impact_statement_risks", "one_stop_english", "semiconductor_org_types", "tai_safety_research", "systematic_review_inclusion", "overruling", "tweet_eval_hate", "twitter_complaints", "ade_corpus_v2", "banking_77", "terms_of_service"]:
            #     answer_choices = load_dataset("ought/raft", name=self.preset_templates[1])["train"].features["Label"].names[1:]
            #     template.answer_choices = ' ||| '.join(answer_choices)
    ...
    def create_raft_jinja(self, dataset_name, answer_choices):
        # with open(os.path.join(os.path.dirname(__file__), "../../../../examples/t_few/raft_prompt_construction_settings.jsonl")) as f:
        with open("/home/ubuntu/ralyaly/autogluon_tfew_ensemble/examples/t_few/raft_prompt_construction_settings.jsonl") as f:
            data = [json.loads(line) for line in f]
            FIELD_ORDERING = data[0]
            INSTRUCTIONS = data[1]
            instruction = INSTRUCTIONS[dataset_name]
            fields = FIELD_ORDERING[dataset_name]
        ...
        for key in fields:
            input_str.append(str(key) + ": " + "{{" + key +  "}}")

        input_str.append("? ".join([choice for index, choice in enumerate(answer_choices)]))
        return '\n'.join(input_str) + "?"
    ...
    def __init__(self, template_config: dict):
        ...
        if self.preset_templates:
            ...
            if self.preset_templates[0]=='raft':
                answer_choices = load_dataset("ought/raft", name=self.preset_templates[1])["train"].features["Label"].names[1:]
                template_jinja = self.create_raft_jinja(self.preset_templates[1], answer_choices)
                print("CHOICES", answer_choices)
                answer_choices = ' ||| '.join(answer_choices)
                print(answer_choices)
                template = Template(0, template_jinja, "custom", answer_choices=answer_choices)
                print("Using Template: ", template.jinja)
    ...
    def get_num_classes(self, dataset_name):
        if dataset_name == "rte":
            ...
            return 2
        elif dataset_name == "neurips_impact_statement_risks":
            return 2
        elif dataset_name == "tai_safety_research":
            return 2
        elif dataset_name == "tweet_eval_hate":
            return 2
        elif dataset_name == "one_stop_english":
            return 3
        elif dataset_name == "systematic_review_inclusion":
            return 2
        elif dataset_name == "twitter_complaints":
            return 2
        elif dataset_name == "overruling":
            return 2
        elif dataset_name == "ade_corpus_v2":
            return 2
        elif dataset_name == "terms_of_service":
            return 2
        elif dataset_name == "semiconductor_org_types":
            return 3
        elif dataset_name == "banking_77":
            return 77

    def get_best_labels(self, dataset_name):
        if dataset_name in ['rte']:
        ...
        elif dataset_name in ["neurips_impact_statement_risks", "one_stop_english", "semiconductor_org_types", "tai_safety_research", "systematic_review_inclusion", "overruling", "tweet_eval_hate", "twitter_complaints", "ade_corpus_v2", "banking_77", "terms_of_service"]:
            answer_choices = load_dataset("ought/raft", name=self.preset_templates[1])["train"].features["Label"].names[1:]
            ...
            if dataset_name == "emotion":
                ...
                return ["not-counterfactual", "counterfactual"] #No Yes
    ...
    def get_raft_labels(self, dataset_name):
        if dataset_name == "neurips_impact_statement_risks":
            return ["doesn't mention a harmful application", "mentions a harmful application"]
        elif dataset_name == "tai_safety_research":
            return ["agent", "learning"]
        elif dataset_name == "tweet_eval_hate":
            return ["Sports", "World"]
        elif dataset_name == "one_stop_english":
            return ["Black", "World", "Science"]
        elif dataset_name == "systematic_review_inclusion":
            return ["included", "not included"]
        elif dataset_name == "twitter_complaints":
            return ["complaint", "no complaint"]#["refund", "send"]
        elif dataset_name == "overruling":
            return ["court", "overrule"]
        elif dataset_name == "ade_corpus_v2":
            return ["chemotherapyinduced", "diagnosis"]
        elif dataset_name == "terms_of_service":
            return ["not potentially unfair", "potentially unfair"]
        elif dataset_name == "semiconductor_org_types":
            return ["company", "research institute", "university"]
        elif dataset_name == "banking_77":
            return load_dataset("ought/raft", name=dataset_name)["train"].features["Label"].names[1:]
            #return ' ||| '.join(answer_choices)

    def get_glue_labels(self, dataset_name):
        if dataset_name == "rte":
            return ["Yes", "No"]
        elif dataset_name == "wsc":
            return ["alice", "susan"]
            ...
            return ["No", "Yes"]
        elif dataset_name == "anli-r1":
            return ["Yes", "B", "No"]
        elif dataset_name == "anli-r2":
            return ["Yes", "C", "No"]
        elif dataset_name == "anli-r3":
            return ["Yes", "no", "No"] #["refund", "send"]
        elif dataset_name == "emotion":
            return ["alone", "joyful", "loving", "irritable", "uncomfortable", "bewildered"]
        elif dataset_name == "enron_spam":
            return ["de", "negative"]
        elif dataset_name == "sst5":
            return ["dull", "trope", "cliche", "tale", "screenplay"]
        elif dataset_name == "sentevalcr":
            return ["issue", "quality"]
        elif dataset_name == "amazon_counterfactual_en":
    ...
    def get_query_variable_info(self, dataset_name):
        if dataset_name in ['wic']:
            ...
            with open("/home/ubuntu/ralyaly/autogluon_tfew_ensemble/examples/t_few/raft_prompt_construction_settings.jsonl") as f:
                ...
                FIELD_ORDERING = data[0]
                INSTRUCTIONS = data[1]
                fields = FIELD_ORDERING[self.preset_templates[1]]
            query = ' and '.join(fields).replace("_", ' ')
            print("Query to retrieve templates: ", query)
            query_elements = fields
            num_variables = len(fields)
    ...
    def find_templates_from_database(self, top_k=5):
        ...
        if self.template_kb == "all":
            ...
            datasets_to_be_considered = [["commonsense_qa", ""], ["dream", ""], ["quail", ""], ["social_i_qa", ""], ["wiki_qa", ""], ["cosmos_qa", ""], ["amazon_polarity", ""], ["imdb", ""], ["yelp_polarity", ""], ["rotten_tomatoes", ""], ["ag_news", ""], ["paws", "labeled_final"], ["wiki_qa", ""], 
            # All from here are not in required format
            ["app_reviews", ""], ["dbpedia_14", ""], ["trec", ""], ["wiki_bio", ""], ["common_gen", ""], ["gigaword", ""], ["multi_news", ""], ["samsum", ""], ["xsum", ""], ["quora", ""], ["quartz", ""], ["wiqa", ""], ["qasc", ""], ["quarel", ""], ["sciq", ""], ["wiki_hop", "original"], ["hotpot_qa", "fullwiki"],
            ]
        ...
        for template in retrieved_templates:
            if self.template_choice_mode == "best":
                template.answer_choices = ' ||| '.join(self.get_best_labels(self.preset_templates[1]))
            elif self.template_choice_mode  in [ "dataset", "auto_dataset"]:
                template.answer_choices = ' ||| '.join(self.get_dataset_labels(self.preset_templates[1]))
            elif self.template_choice_mode in ["auto"]:
                 template.answer_choices = " ||| ".join(self.custom_answer_choices)
            elif self.template_choice_mode in ["raft"]:
                 template.answer_choices = ' ||| '.join(self.get_raft_labels(self.preset_templates[1]))
            elif self.template_choice_mode in ["superglue"]:
                 template.answer_choices = ' ||| '.join(self.get_glue_labels(self.preset_templates[1]))
            elif self.template_choice_mode in ["random"]:
                vocabulary = set([])
                for entry in self.train_data:
                    vocabulary |= set(entry.split())
        ...
        for template in self.templates:
            ...
            # setattr(template, "jinja", template.jinja.replace("True or False?", "").replace("Yes or no?", "").replace("Yes or No?", "").replace("True or False", "").replace("Paraphrase or not?", "").replace("Yes or No.\n", ""))
