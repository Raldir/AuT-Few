...
TEMPLATES_FOLDER_PATH = pkg_resources.resource_filename(__name__, "templates_w_setfit")
...
def download_sourceprompt_templates():
    ...
    temporary_zip_file = pkg_resources.resource_filename(__name__, "templates_w_setfit.zip")

    # with open(temporary_zip_file, mode="rb") as f:
...
class Template(yaml.YAMLObject):
    ...
    def get_template_fields(self):
        ...
        """
        input_text = re.findall(r"\{\{(.+?)\}\}", str(self.jinja).split("|||")[0])
        fields = []
        answers_in_input = []
        for variable in input_text:
            variable = variable.strip()
            if "answer_choices" not in variable and "answer[" not in variable:
                fields.append(variable)
            else:
                answers_in_input.append(variable)

        output_text = re.findall(r"\{\{(.+?)\}\}", str(self.jinja).split("|||")[1])
        answers = []
        for variable in output_text:
            variable = variable.strip()
            answers.append(variable)
            # if "answer_choices" not in variable and "answer[" not in variable:
            #     print("EEEERRROR", self.jinja)
            # else:
            #     answers.append(variable)
        
        # return ([x.strip() for x in re.findall(r"\{\{(.+?)\}\}", str(self.jinja)) if "answer_choices" not in x.strip()], [x.strip() for x in re.findall(r"\{\{(.+?)\}\}", str(self.jinja)) if "answer_choices" in x.strip()])
        return [fields, answers_in_input, answers]
