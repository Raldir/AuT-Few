...
from ..constants import AUTOMM, LM_TARGET, LOGITS, T_FEW, TEMPLATE_LOGITS, WEIGHT, BEST_K_MODELS_FILE
...
class LitModule(pl.LightningModule):
    ...
    def on_fit_end(self):
            # current_score = self.validation_metric.compute()
            # logger.info(f'Current {self.validation_metric_name} : {current_score}')
            # self.validation_metric.reset()
            trainable_states = {
                param_name: param_weight.cpu()
                for param_name, param_weight in self.model.state_dict().items()
                if param_name in self.finetunable
            }

            checkpoint_path = f'epoch_{self.current_epoch}_{self.validation_metric_name}_0.ckpt'
            self.best_models[os.path.join(self.save_path, checkpoint_path)] = 0.0
            print("HERE", self.save_path)
            with open(os.path.join(self.save_path, BEST_K_MODELS_FILE), 'w') as outfile:
                yaml.dump(self.best_models, outfile)
            trainable_states = {"state_dict": trainable_states}
            torch.save(trainable_states, os.path.join(self.save_path, checkpoint_path))
            

            # if not self.best_models or current_score >= self.best_models[list(self.best_models.keys())[-1]]:
            #     checkpoint_path = f'epoch_{self.current_epoch}_{self.validation_metric_name}_{current_score}.ckpt'
            #     self.best_models[os.path.join(self.save_path, checkpoint_path)] = float(current_score)
            #     print("HERE", self.save_path)
            #     with open(os.path.join(self.save_path, BEST_K_MODELS_FILE), 'w') as outfile:
            #         yaml.dump(self.best_models, outfile)
            #     trainable_states = {"state_dict": trainable_states}
            ...
            #     logger.info(f"New best score: {str(current_score)} at epoch {self.current_epoch}.")
            #     self.best_scores = {self.current_epoch : str(current_score.item())}
            #     with open(os.path.join(self.save_path, "validation_score.txt"), 'w') as outfile:
            #         yaml.dump(self.best_scores, outfile)
    ...
    def _compute_template_loss(
        ...
        if self.model.unlikely_loss > 0:
            ...
            cand_loglikely += (lm_target < 0).view(bs, num_choices, -1) * self.model.padding_token
            cand_loglikely[range(bs), label] = self.model.padding_token
            unlikely_loss = -torch.log(1 - torch.exp(cand_loglikely) + 1e-2).sum() / (cand_loglikely != self.model.padding_token).sum()
        else:
            unlikely_loss = 0.0
    ...
    def configure_optimizers(self):
        ...
        if self.hparams.lr_choice == "two_stages":
            ...
            grouped_names, grouped_parameters = apply_layerwise_lr_decay(
